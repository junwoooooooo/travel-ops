...
첫 배포에서 이 단계가 함정을 하나 드러냈다. 원격 스크립트는 ssh stdin(heredoc)으로 흘러
들어와 `bash -s`가 한 줄씩 읽는 중인데, `docker compose run`이 컨테이너에 stdin을 붙이면서
**남은 스크립트를 그대로 삼켰다.** 마이그레이션은 성공했고 job도 0으로 끝났는데 `up -d`는
실행조차 되지 않아 구 컨테이너가 그대로 떠 있었다. `-T`와 `</dev/null`을 붙여 stdin을 끊는다.
잡아낸 것은 세션 7에서 만든 커밋 대조 검증이다 — "200이 온다"만 봤으면 통과했을 배포다.
